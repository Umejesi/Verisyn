...
function reflectAuthUI(){
  ...
  if(isLoggedIn){
    ...
    document.getElementById('googleSigninLink').addEventListener('click', (e)=>{
      e.preventDefault();
      chrome.tabs.create({ url: `${SITE_URL}/login.html` });
    });
  }
}
...
function renderResults(analysis, address, chain, planTier, remaining){
  ...
  document.getElementById('communityRow').innerHTML = `
    <button data-report="safe">✓ Safe</button>
    <button data-report="suspicious">? Suspicious</button>
    <button data-report="scam">⚠ Scam</button>`;
  document.querySelectorAll('#communityRow button').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      getGuestId(async (guestId)=>{
        try{
          await fetch(`${SITE_URL}/api/community`, {
            method:'POST', credentials:'include', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ address, report: btn.dataset.report, guestId })
          });
          btn.textContent = '✓';
...
document.getElementById('watchBtn').addEventListener('click', async ()=>{
  ...
  if(!isPro){
    ...
    document.getElementById('quotaLine').textContent = 'Watchlists with alerts are a Pro feature — upgrade on the site.';
    return;
  }
  const { address, chain } = lastAnalysis;
  const label = `${address.slice(0,6)}…${address.slice(-4)}`;
  const btn = document.getElementById('watchBtn');
  try{
    await fetch(`${SITE_URL}/api/watchlist`, {
      method:'POST', credentials:'include', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ address, chain, label })
    });
    const orig = btn.textContent; btn.textContent = '✓ Saved — alerts on';
    setTimeout(()=>btn.textContent=orig, 1800);
  }catch(e){
    btn.textContent = 'Could not save — try again';
    setTimeout(()=>btn.textContent='⭐ Save to watchlist', 1800);
  }
});
